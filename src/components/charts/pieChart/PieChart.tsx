...
export type PieProps = {
  ...
  animate?: boolean;
  ordinalColorScale: unknown;
};

export default function PieChart({ chartData, total, width, height, scale, margin = defaultMargin }: PieProps) {
        ...
        <Group top={centerY + margin.top} left={centerX + margin.left}>
          ...
          {active ? (
            <>
              ...
              <Text textAnchor="middle" fill={active.color} className="text-lg" dy={0}>
                {`${active.label}`}
              </Text>
            </>
          ) : (
